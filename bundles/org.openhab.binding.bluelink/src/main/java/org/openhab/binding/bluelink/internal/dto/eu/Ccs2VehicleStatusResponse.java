...
public record Ccs2VehicleStatusResponse(String resCode, @SerializedName("ServiceNo") String serviceNo,
    ...
    private static boolean isOpen(final @Nullable OpenState state) {
        return state != null && state.open() > 0;
    }

    private static <T> int[] extract(final @Nullable FrontRow<T> row1, final @Nullable GenericRow<T> row2,
            final ToIntFunction<T> mapper) {
        final int frontLeft = row1 != null && row1.driver() != null ? mapper.applyAsInt(row1.driver()) : 0;
        final int frontRight = row1 != null && row1.passenger() != null ? mapper.applyAsInt(row1.passenger()) : 0;
        final int backLeft = row2 != null && row2.left() != null ? mapper.applyAsInt(row2.left()) : 0;
        final int backRight = row2 != null && row2.right() != null ? mapper.applyAsInt(row2.right()) : 0;
        return new int[] { frontLeft, frontRight, backLeft, backRight };
    }
    ...
    public CommonVehicleStatus toCommonVehicleStatus(IVehicle vehicle) {
        ...
        return new CommonVehicleStatus() {
            ...
            public boolean doorLock() {
                final var door = v.cabin().door();
                if (door == null) {
                    return false;
                }
                for (final int value : extract(door.row1(), door.row2(), DoorState::lock)) {
                    if (value > 0) {
                        return true;
                    }
                }
            ...
            public DoorStatus doorOpen() {
                ...
                final int[] states = extract(door.row1(), door.row2(), DoorState::open);
                return new DoorStatus(states[0], states[1], states[2], states[3]);
            }

            @Override
            public DoorStatus windowOpen() {
                final var window = v.cabin().window();
                if (window == null) {
                    return new DoorStatus(0, 0, 0, 0);
                }
                final int[] states = extract(window.row1(), window.row2(), WindowState::open);
                return new DoorStatus(states[0], states[1], states[2], states[3]);
            }
            ...
            public SeatHeaterState seatHeaterVentState() {
                final var seat = v.cabin().seat();
                if (seat == null) {
                    return new SeatHeaterState(0, 0, 0, 0);
                }
                final int[] states = extract(seat.row1(), seat.row2(),
                        state -> state.climate() != null ? state.climate().state() : 0);
